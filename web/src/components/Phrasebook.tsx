...
// character said. It is opened from the link above the input bar and rises as a sheet
// from the foot of the screen, centred on the same column as everything else.
...
export function Phrasebook() {
  ...
  return (
    <div className="absolute inset-0 z-30" data-testid="phrasebook">
      <button type="button" aria-label="Close phrasebook" onClick={() => setOpen(false)} className="fade-in absolute inset-0 h-full w-full cursor-default border-0 bg-black/50" />
      <aside
        aria-label="Phrasebook"
        className="sheet-in pointer-events-auto absolute inset-x-0 bottom-0 mx-auto flex max-h-[82%] w-full max-w-[560px] flex-col rounded-t-[14px] border border-b-0 border-hair bg-[#0e0f0f]/96 backdrop-blur-xl"
      >
